...
/* =========================
   CANVAS PAGE (canvas.html)
   Full artwork first.
   Begin placement enables zoom/pan/select.
   After placing, visitor can still explore but cannot place again.
========================= */
...
(async function initCanvasPage() {
  ...
  let chosenColor = localStorage.getItem("omm_color") || "#1a1a1a";
  let hoverCell = null;
  ...
  function drawSelectedCell() {
    ...
    ctx.fillStyle = hexToRgba(chosenColor, 0.38);
    ctx.fillRect(sx, sy, cp, cp);
  ...
  function enterPlacementMode() {
    ...
    if (startFromSharedLink) {
      scale = Number.isFinite(urlZRaw) ? clamp(urlZRaw, 0.2, 5) : 1.2;
      clampCamera();
      ...
      if (jumpNoteEl) {
        jumpNoteEl.textContent = `Viewing coordinates (${Math.round(startFromSharedLink.x)}, ${Math.round(startFromSharedLink.y)}).`;
      }
    } else {
      clampCamera();
    }
    ...
    beginBtnEl.textContent = hasPlacedMyMark ? "Exploring artwork" : "Placement mode";
    ...
    if (!hasPlacedMyMark) {
      ...
      if (confirmNoteEl) {
        confirmNoteEl.textContent = "Drag to navigate. Scroll to zoom. Click a cell to choose.";
      }
    } else {
      confirmBtnEl.style.display = "none";

      if (confirmNoteEl) {
        confirmNoteEl.textContent = "Drag to navigate. Scroll to zoom. Your mark has already been placed.";
      }
    }
  ...
  function selectCell() {
    ...
    if (hoverCell.x < 0 || hoverCell.y < 0 || hoverCell.x >= GRID_COLS || hoverCell.y >= GRID_ROWS) {
      if (confirmNoteEl) confirmNoteEl.textContent = "This point lies outside the artwork.";
      return;
    }

    if (cellTaken(hoverCell.x, hoverCell.y)) {
      if (confirmNoteEl) confirmNoteEl.textContent = "This position is already occupied.";
      return;
    }
    ...
    if (coordsTextEl) {
      coordsTextEl.textContent = `Selected coordinates: (${selectedCell.x}, ${selectedCell.y})`;
    }

    if (confirmNoteEl) {
      confirmNoteEl.textContent = "You may choose another position, or make this mark permanent.";
    }
  ...
  function loadMyMark() {
    ...
    const color = localStorage.getItem(LS_COLOR) || "#1a1a1a";
    ...
    beginBtnEl.textContent = "Explore the artwork";
    beginBtnEl.classList.remove("disabled");

    confirmBtnEl.textContent = "Mark placed";
    confirmBtnEl.classList.add("disabled");
    confirmBtnEl.style.display = "none";

    if (statusTextEl) statusTextEl.textContent = "Your mark is now part of this artwork.";
    if (coordsTextEl) coordsTextEl.textContent = `Your coordinates: (${x}, ${y})`;
  ...
  function confirmPlacement() {
    ...
    if (coordsTextEl) coordsTextEl.textContent = `Your coordinates: (${x}, ${y})`;
    if (archiveTextEl) archiveTextEl.textContent = "Archival publication is not yet enabled.";
    if (confirmNoteEl) confirmNoteEl.textContent = "You may still explore the artwork. A second mark cannot be placed.";
    ...
    // After placement: zoom out and show surrounding context, not exactly “spotlighted”
    scale = 0.85;
    setCameraCenteredOn(x + 12, y - 8);
    placementMode = true;
  ...
  function movePan(event) {
    ...
    cameraX = panStartCamera.x - dxPx / cellPx();
    cameraY = panStartCamera.y - dyPx / cellPx();
  ...
  function handleJump() {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y)) {
      if (jumpNoteEl) jumpNoteEl.textContent = "Enter numeric coordinates.";
      return;
    }
    ...
    if (jumpNoteEl) jumpNoteEl.textContent = `Viewing coordinates (${target.x}, ${target.y}).`;
  ...
  function getCurrentViewForCopy() {
    ...
    if (placementMode) {
      return {
        x: cameraX + visibleCols() / 2,
        y: cameraY + visibleRows() / 2,
        z: scale,
      };
    }
  ...
  try {
    ...
    for (const m of data.marks || []) {
      canonicalMarks.set(markKey(m.x, m.y), m.color || "#1a1a1a");
    }
  } catch (err) {
    console.warn(err);
  }
  ...
  beginBtnEl.addEventListener("click", enterPlacementMode);
  ...
  if (jumpXEl) {
    jumpXEl.addEventListener("keydown", (e) => {
      if (e.key === "Enter") handleJump();
    });
  }

  if (jumpYEl) {
    jumpYEl.addEventListener("keydown", (e) => {
      if (e.key === "Enter") handleJump();
    });
  }
  ...
  // DEV MODE: open canvas.html?dev=1 to reset local mark
